...
function get_accessible_books_for_user($user) {
    ...
    $stmt = db()->prepare("SELECT id, title, subject, grade, major FROM books WHERE grade=? AND (major='all' OR major=?) ORDER BY subject ASC, title ASC");
    $stmt->execute([(int)$user['grade'], $major]);
...
function user_can_access_book($user, $book) {
    ...
    $bookMajor = $book['major'] ?? 'all';
    return (int)$book['grade'] === (int)$user['grade'] && ($bookMajor === 'all' || $bookMajor === normalize_major($user['major'] ?? 'math'));
}
